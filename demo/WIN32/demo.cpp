...
static UCHAR    ucRegHoldingBuf[REG_HOLDING_NREGS];
...
_tmain( int argc, _TCHAR * argv[] )
{
	...
	{
		//usRegHoldingBuf[Num] = ((Num + 1) * 2);
		usRegHoldingBuf[Num] = 0;

	}
    const UCHAR     ucSlaveID[] = { 0xAA, 0xBB, 0xCC };

    if( eMBInit( MB_RTU, 0x0A, 3, 38400, MB_PAR_EVEN ) != MB_ENOERR )
    {
    ...
    else
    {
        ...
        do
        {
            ...
            case _TCHAR( 'q' ):
                ...
                break;
			case _TCHAR( 'r' ):	
				//if (eMBFuncReadHoldingRegister(ucRegHoldingBuf, &usLen))
				//{
				//}
				//if (eMBRTUSend(0x0A, ) == MB_ENOERR)
				//{
				//}

				//if (eMBRegHoldingCB(ucRegHoldingBuf, REG_HOLDING_START, usLen, MB_REG_READ) == MB_ENOERR)
				//{
				//	//MessageBox(NULL, (LPCSTR)ucRegHoldingBuf, "信息提示", MB_OK|MB_ICONINFORMATION);
				//}
				break;
            case _TCHAR( 'd' ):
            ...
            case _TCHAR( 'h' ):
				...
				_tprintf( _T( "  'r' ... read information from slaver\r\n" ) );
